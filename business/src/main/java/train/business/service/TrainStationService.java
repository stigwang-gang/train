...
import train.business.domain.TrainStation;
import train.business.domain.TrainStationExample;
import train.business.mapper.TrainStationMapper;
...
public class TrainStationService {
    ...
    public void save(TrainStationSaveReq req) {
        ...
        if (ObjectUtil.isNull(trainStation.getId())) {
            ...
            TrainStation existingTrainStation = selectByUnique(req.getTrainCode(), req.getIndex());
            if (ObjectUtil.isNotEmpty(existingTrainStation) && !existingTrainStation.getId().equals(trainStation.getId())) {
                // 如果存在重复记录且主键不同，抛出业务异常
                throw new BusinessException(BusinessExceptionEnum.BUSINESS_TRAIN_STATION_UNIQUE_ERROR);
            }
            trainStation.setUpdateTime(now);
